...
| `TODO` | **Expanded Item Bag** | Items pocket expanded from 42 → 64 slots (save-breaking; `BAG_ITEMS_COUNT`) |
| `DONE` | **Auto-Run Toggle** | `FLAG_AUTO_RUN` (flag `0x4AB`): B-button press toggles between run/walk mode (works while moving or standing). Movement logic in `field_player_avatar.c` uses flag only; toggle wired in `field_control_avatar.c` with audio feedback (`SE_PC_ON`/`SE_PC_OFF`) |
